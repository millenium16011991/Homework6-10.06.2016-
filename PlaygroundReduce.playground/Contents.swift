...
var concatination: ((String?, String?) -> String)

func concatinationFunction(first: String?, second: String?) -> String {
    guard let name = first, let model = second else{
        return ""}
    return name+model
}
concatination = concatinationFunction
concatination(car1.driver.name, car1.model)
concatination(car2.driver.name, car2.model)
concatination(car3.driver.name, car3.model)
concatination(car4.driver.name, car4.model)
concatination(car5.driver.name, car5.model)
